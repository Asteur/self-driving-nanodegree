...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
	...
	// to handle 100ms delay, actuation is chosen with delay of 100ms, i.e. 100ms/0.05s = 2.
	vector<double> response;

	response.push_back(solution.x[delta_start+2]);
	response.push_back(solution.x[a_start+2]);

	for (int i = 0; i < N - 1; i++) {
		response.push_back(solution.x[x_start + i + 1]);
		response.push_back(solution.x[y_start + i + 1]);
	}
